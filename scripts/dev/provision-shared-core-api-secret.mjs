...
const variableName = 'SHARED_CORE_DATABASE_URL';
...
const alter = spawnSync(
  ...
  {
    input: "ALTER ROLE shared_core_api PASSWORD :'runtime_password';\n",
    encoding: 'utf8',
...
if (alter.status !== 0) {
  process.stderr.write('Failed to set the Shared Core API password.\n');
...
const value = `postgresql://shared_core_api:${encodeURIComponent(password)}@127.0.0.1:5433/shared_core`;
...
if (verify.status !== 0 || verify.stdout.trim() !== 'shared_core|shared_core_api') {
  process.stderr.write('Shared Core API TCP authentication verification failed.\n');
